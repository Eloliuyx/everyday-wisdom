...
await writeFile(path.join(target, 'README.md'), '# Everyday Wisdom — test vault\n\nThis vault contains test notes only. Open this folder as a vault in Obsidian, then enable the bundled Everyday Wisdom community plugin if prompted. Daily notes is already configured.\n\n1. Turn Automatic insertion on, then create a new daily note: expect one reflection below the properties. Opening an already existing daily note does not add anything.\n2. Reopen it: expect no duplicate.\n3. Open Daily/2025-03-01: expect no automatic change. Use Backfill if you want to add its March 1 reflection. There is no individual insertion command.\n4. Use Backfill with All existing daily notes, preview, then confirm. No missing dates should be created.\n5. Edit text inside one reflection, then use Deletion. The edited block and all personal writing should remain. Automatic insertion should turn off by default. Turning the switch back on should affect only notes created afterwards.\n\nFull test matrix: the parent repository’s docs/TESTING.md.\n');
